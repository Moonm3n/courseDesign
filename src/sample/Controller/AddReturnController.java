...
public class AddReturnController {
    ...
    public void yes() {
        ...
        if (book == null) {
            ...
            text = "读者ID错误，请检查";
        } else if (reader.getBorrowedBook() <= 0) {
            text = "读者未借书，请检查";
        ...
        updateSql = "UPDATE `borrow_info` SET  `status` = '2' where book_id = '" + bookIdTextField.getText()
                + "' AND reader_id = '" + readerIdTextField.getText() + "' AND `status` = '1' ";
        JDBCDao.insertOrDeleteOrUpdate(updateSql);

        StringBuilder sql = new StringBuilder();
        sql.append("INSERT INTO `return_info` (`book_id`, `reader_id`, `return_time`) VALUES ( ");
